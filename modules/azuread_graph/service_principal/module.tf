...
resource "azuread_service_principal" "serp" {
  application_id               = can(var.settings.azuread_application.key) == false ? try(var.settings.azuread_application.id, data.azuread_application_published_app_ids.well_known.result[var.settings.azuread_application.well_known_key]) : try(var.remote_objects.azuread_graph_applications[var.settings.azuread_application.key].application_id, var.remote_objects.azuread_applications[try(var.settings.azuread_application.lz_key, var.client_config.landingzone_key)][var.settings.azuread_application.key].application_id)
