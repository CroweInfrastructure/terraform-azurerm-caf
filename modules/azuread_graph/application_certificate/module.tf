...
resource "azuread_application_certificate" "appc" {
  application_object_id = can(var.settings.azuread_application_object.key) == false ? var.settings.azuread_application_object.id : var.remote_objects.azuread_applications[try(var.settings.azuread_application_object.lz_key, var.client_config.landingzone_key)][var.settings.azuread_application_object.key].object_id
