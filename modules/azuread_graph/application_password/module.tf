...
resource "azuread_application_password" "appp" {
  application_object_id = can(var.settings.azuread_application_object.key) == false ? try(var.settings.azuread_application_object.id, data.azuread_application_published_app_ids.well_known.result[var.settings.azuread_application.well_known_key]) : var.remote_objects.azuread_applications[try(var.settings.azuread_application_object.lz_key, var.client_config.landingzone_key)][var.settings.azuread_application_object.key].application_id
